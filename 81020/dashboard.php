...
<!--.modal {
  ...
  display: block !important;
	...
	echo $countm = $statement->rowCount();
                                    ...
                                    <div class="sd-title">
									 ...
									 <div class="suggestions-list">
									   <ul class="social_links MyList">
                                        ...
                                        <li data-tab="review"><a href="company-list.php" title=""><i class="fa fa-file-text-o" aria-hidden="true"></i>View Pages</a></li>
										<li><a href="company-page.php" title=""><i class="fa fa-file" aria-hidden="true"></i>Create Pages</a></li>
                                        <li ><a href="profile-account-setting.php" title="" class="animated fadeIn active" ><i class="fa fa-cogs" aria-hidden="true"></i>Settings</a></li>
							...
							<div class="col-lg-6 col-md-8 col-sm-12 col-12 no-pd dashboardtimeline">
...
<div id="msgbox">
										...
										<div class="postoptions">
											...
											<div class="create-post postoptions">
  ...
  <input type="hidden" id="postid1" value="<?=$row['post_id']?>">
				                    <div class="contact contact-rounded contact-lg" id="commdel<?=$rowc['c_id']?>">
				                        ...
				                        <div class="contact-container">
				                            ...
				                            <div class="replycommnetbt">
												...
												<a  href="javascript:void(0)" id="editcomment" class="editcomment editcomment1" cid="<?=$rowc['c_id']?>" ><i class="edit-icon fa fa-pencil"></i></a>
								           	</div>
				                        </div>
				                    </div>
				                    ...
				                    <?php ///////Show Reply////////
											...
											$rcomallcount=$db1->getSingleResult("SELECT count(*) from reply where c_id=".$rowc['c_id']);
											 $sqlr="SELECT * from reply where c_id=".$rowc['c_id']." limit 0,5";
											$dbr->query($sqlr);
											if($dbr->numRows()>0)
											{
					...
					<div class="form newpostcomments">
	                    <div class="form-group">
	                        	...
	                        	<form id="" method="post">
													...
													<a href="javascript:void(0);" name="send_chatemoji1"  class="send_chatemoji1" id="comment1<?=$row['post_id']?>" uid="<?=$row['post_id']?>" cid="<?=$row['post_id']?>"><i class="emoji-picker-icon emoji-picker fa fa-smile-o" style="margin-right: 44px; margin-top: 2px;"></i> </a>
										...
										<div class="post-bar">
											...
											<?php if(!empty($userrow['current_company']) or !empty($row['livelocation']) or !empty($userrow['current_city'])){?>
												...
												<?php if(!empty($userrow['current_company'])){?>
													...
													<?php } if(!empty($row['livelocation'])){?>
													<li><img src="images/icon9.png" alt=""><span><?=$row['livelocation']?></span></li><?php }elseif(!empty($userrow['current_city'])){?>
...
$dbmost1=new DB();										
										$db7=new DB();										
										$sqlmost="SELECT * FROM user_profile  order by search desc limit 0,10";
										$dbmost->query($sqlmost);
										if($dbmost->numRows()>0)
										{
										while($mostrow=$dbmost->fetchArray()){
										$u=$dbmost1->getSingleResult("select user_id from followers where follow=".$mostrow['user_id']);
											//if($u >0){}else{
											?>
												
												<div class="user-profy">
												<?php if($mostrow['image_id']){?>
													<a href="view-profile.php?uid=<?php echo base64_encode($mostrow['user_id']);?>"> <img src="upload/<?=$mostrow['image_id']?>" alt="" style="" ></a>
                                                   <?php }else{ ?>
                                                <a href="view-profile.php?uid=<?php echo base64_encode($mostrow['user_id']);?>"><img src="images/resources/user.png" alt="" style=""></a>
												
												<?php } ?>
													
													<h3><?=$mostrow['first_name']?></h3>
													<?php if(!empty($mostrow['current_company'])){ ?> 
													<span><?=$mostrow['current_company']?></span>
													<?php }else{
													    echo "<span>N/A</span>";
													
													} ?>
													<ul>
													<?php $cnt=$db7->getSingleResult("SELECT count(f_id) from followers where user_id = '".$_SESSION['sess_webid']."' and follow=".$mostrow['user_id']);
							
													$f_id=$db7->getSingleResult("SELECT f_id from followers where user_id = '".$_SESSION['sess_webid']."' and follow=".$mostrow['user_id']);
													if($cnt>0){
													?>
														<!--<li><a href="#" title="" class="followw">Follow</a></li>-->
														<li><a class="clrbtn follownew1"  id="<?=$f_id?>" fid="<?=$f_id?>" href="javascript:void(0);"> Unfriend</a></li>
													<?php }else{ ?>
													<li>
													<a class="clrbtn sendreequest"  id="sendreequest<?=$mostrow['uder_id']?>"   fid="<?=$mostrow['uder_id']?>" href="javascript:void(0);">Add as a Friend</a>
													<!--<a class="clrbtn follownew"  id="<?=$mostrow['user_id']?>" fid="<?=$mostrow['user_id']?>" href="javascript:void(0);"> Add as a Friend</a>--></li>
													<?php } ?>
														
														<!--<li><a href="#" title="" class="envlp"><img src="images/envelop.png" alt=""></a></li>-->
														
													</ul>
													<a href="view-profile.php?uid=<?=base64_encode($mostrow['user_id'])?>" title="">View Profile</a>
												</div><!--user-profy end-->
		...
		<div id="load_data_message"></div>
							...
							<div class="col-lg-2 col-md-2 pd-right-none no-pd pointbarmain">
								<div class="right-sidebar">
									<div class="widget widget-about full-width">
...
echo $reward=$dbrew->getSingleResult('select reward from '.$_TBL_USER." where user_id=".$_SESSION['sess_webid']); ?></h3>
						...
						<?php }} ?>		 			
									...
									<div class="widget suggestions full-width">
										...
										<div class="suggestions-list overflow-auto mostViewH">
										<?php
$db22=new DB();
$l22=array();
$sql22="SELECT * from followers where user_id=".$_SESSION['sess_webid'];
$db22->query($sql22);
if($db22->numRows()>0)
{
while($row22=$db22->fetchArray()){
	$l22[]=$row22['follow'];
}
}
$allfriend22=implode(',',$l22);
if(!empty($allfriend22)){
$allfriends2=$allfriend22;
}else{
$allfriends2=0;	
}
										...
										$sqln="SELECT * FROM user_profile where user_id NOT IN($allfriends2)  order by search desc limit 0,10";
										$db->query($sqln);
										if($db->numRows()>0)
										{
										while($vrow=$db->fetchArray()){
											...
											<?php if(!empty($vrow['image_id'])){?>
												<img src="upload/<?=$vrow['image_id']?>" alt="" >
											<?php }else{?>
											  <img src="images/resources/user.png" alt="" >
											<?php } ?>
												<div class="sgt-text">
			<a href="view-profile.php?uid=<?=$uid1?>"><h4><?=$vrow['first_name']?></h4></a>
										...
										<?php } }else{ echo "<p>Data Not Available<p>";} //} ?>
											<!--<div class="view-more">
...
"; */
//$query = "SELECT * FROM login WHERE user_id != '".$_SESSION['user_id']."' and f_userid IN($allfriend)";
 $query = "SELECT * FROM login WHERE user_id != '".$_SESSION['user_id']."' and f_userid IN (SELECT f_userid FROM login_details
...
/* 	echo $query1 = "
	...
	WHERE f_userid = '".$rowchat['f_userid']."' and status='Online'";
	$statement1 = $connect->prepare($query1);
	$statement1->execute();
	echo $result1 = $statement1->fetchAll(); */
	//if($result1 > 0){
	/////////////////////
	$status = '';
...
//console.log(result);
	...
	<style>
.wishlistcartemoji1{ width: 300px !important; background: #fff;  position: absolute;    }
...
if($ext=='mp3'){
 ...
 <div class="overview-box" id="editcomment-box">
        <div class="overview-edit" id="commedit">
...
$(document).ready(function(){
    ...
    $(document).scroll(function(e){
        ...
        if ($(window).scrollTop() >= ($(document).height() - $(window).height())*0.7){
            processing = true;
            $.post('/echo/html/', 'html=<div class="loadedcontent"></div>', function(data){
                $('#containernew').append(data);
                processing = false;
            });
        }
    });
...
/* 
	$(".showcmtBtn").click(function(){ 
		$(this).parents("content").children(".mainCommentList").slideToggle();
		//$(".content a").not(this).hide("slow");
	});
 */
